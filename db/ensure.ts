...
import { DEFAULT_PAYMENT_ACCOUNTS, paymentAccountKey } from "../app/payment-accounts";
...
export function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    await d1.batch([
      ...
      d1.prepare("CREATE TABLE IF NOT EXISTS expenses (id text PRIMARY KEY NOT NULL, expense_no text NOT NULL, order_id text NOT NULL, person_id text NOT NULL, vendor_id text DEFAULT '' NOT NULL, category text NOT NULL, vendor text NOT NULL, description text NOT NULL, expense_date text NOT NULL, amount integer NOT NULL, payment_mode text NOT NULL, receipt_key text DEFAULT '' NOT NULL, receipt_name text DEFAULT '' NOT NULL, funding_source text DEFAULT 'Reimbursement' NOT NULL, payment_account_id text DEFAULT '' NOT NULL, payment_account_name text DEFAULT '' NOT NULL, status text DEFAULT 'Pending approval' NOT NULL, approved_at text DEFAULT '' NOT NULL, approved_by text DEFAULT '' NOT NULL, disapproved_at text DEFAULT '' NOT NULL, disapproved_by text DEFAULT '' NOT NULL, reimbursed_amount integer DEFAULT 0 NOT NULL, created_at text NOT NULL)"),
      ...
      d1.prepare("CREATE TABLE IF NOT EXISTS payment_accounts (id text PRIMARY KEY NOT NULL, name text NOT NULL, name_key text NOT NULL, status text DEFAULT 'Active' NOT NULL, created_at text NOT NULL)"),
      d1.prepare("CREATE UNIQUE INDEX IF NOT EXISTS payment_accounts_name_key_unique ON payment_accounts (name_key)"),
      d1.prepare("CREATE TABLE IF NOT EXISTS payments (id text PRIMARY KEY NOT NULL, order_id text DEFAULT '' NOT NULL, manual_order_id text DEFAULT '' NOT NULL, person_id text DEFAULT '' NOT NULL, vendor_id text DEFAULT '' NOT NULL, invoice_id text DEFAULT '' NOT NULL, customer_id text DEFAULT '' NOT NULL, direction text NOT NULL, amount integer NOT NULL, payment_date text NOT NULL, method text NOT NULL, reference text DEFAULT '' NOT NULL, notes text DEFAULT '' NOT NULL, payment_account_id text DEFAULT '' NOT NULL, payment_account_name text DEFAULT '' NOT NULL, receipt_key text DEFAULT '' NOT NULL, receipt_name text DEFAULT '' NOT NULL, created_at text NOT NULL)"),
    ]);
    ...
    for (const column of [["order_id", "text DEFAULT '' NOT NULL"], ["manual_order_id", "text DEFAULT '' NOT NULL"], ["person_id", "text DEFAULT '' NOT NULL"], ["vendor_id", "text DEFAULT '' NOT NULL"], ["payment_account_id", "text DEFAULT '' NOT NULL"], ["payment_account_name", "text DEFAULT '' NOT NULL"], ["receipt_key", "text DEFAULT '' NOT NULL"], ["receipt_name", "text DEFAULT '' NOT NULL"]] as const) if (!names.has(column[0])) await d1.prepare(`ALTER TABLE payments ADD COLUMN ${column[0]} ${column[1]}`).run();
    ...
    for (const column of [["vendor_id", "text DEFAULT '' NOT NULL"], ["funding_source", "text DEFAULT 'Reimbursement' NOT NULL"], ["payment_account_id", "text DEFAULT '' NOT NULL"], ["payment_account_name", "text DEFAULT '' NOT NULL"], ["status", "text DEFAULT 'Pending approval' NOT NULL"], ["approved_at", "text DEFAULT '' NOT NULL"], ["approved_by", "text DEFAULT '' NOT NULL"], ["disapproved_at", "text DEFAULT '' NOT NULL"], ["disapproved_by", "text DEFAULT '' NOT NULL"], ["reimbursed_amount", "integer DEFAULT 0 NOT NULL"]] as const) if (!expenseNames.has(column[0])) await d1.prepare(`ALTER TABLE expenses ADD COLUMN ${column[0]} ${column[1]}`).run();
    const seededAt = new Date().toISOString();
    await d1.batch(DEFAULT_PAYMENT_ACCOUNTS.map((account) => d1.prepare("INSERT OR IGNORE INTO payment_accounts (id, name, name_key, status, created_at) VALUES (?, ?, ?, 'Active', ?)").bind(account.id, account.name, paymentAccountKey(account.name), seededAt)));
