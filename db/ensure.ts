...
export function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    await d1.batch([
      ...
      d1.prepare("CREATE TABLE IF NOT EXISTS vendor_products (id text PRIMARY KEY NOT NULL, vendor_id text NOT NULL, name text NOT NULL, pricing_basis text NOT NULL, rental_charge integer NOT NULL, status text DEFAULT 'Active' NOT NULL, created_at text NOT NULL)"),
      d1.prepare("CREATE TABLE IF NOT EXISTS orders (id text PRIMARY KEY NOT NULL, order_no text NOT NULL, title text NOT NULL, customer_id text NOT NULL, salesperson_id text DEFAULT '' NOT NULL, assigned_person_id text NOT NULL, venue text NOT NULL, event_date text NOT NULL, status text DEFAULT 'Planned' NOT NULL, contract_value integer DEFAULT 0 NOT NULL, created_at text NOT NULL)"),
      d1.prepare("CREATE UNIQUE INDEX IF NOT EXISTS orders_order_no_unique ON orders (order_no)"),
      d1.prepare("CREATE TABLE IF NOT EXISTS order_vendors (id text PRIMARY KEY NOT NULL, order_id text NOT NULL, vendor_id text NOT NULL, product_id text DEFAULT '' NOT NULL, product_name text NOT NULL, pricing_basis text DEFAULT 'Per event' NOT NULL, unit_rate integer DEFAULT 0 NOT NULL, quantity integer DEFAULT 1 NOT NULL, rental_days integer DEFAULT 1 NOT NULL, amount integer NOT NULL, notes text DEFAULT '' NOT NULL, created_at text NOT NULL)"),
