...
export async function createUser(input: NewUser) {
  ...
  await db.prepare("INSERT INTO users (id, name, email, role, status, password_hash, must_change_password, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
    .bind(user.id, user.name, user.email, user.role, user.status, user.passwordHash, user.mustChangePassword ? 1 : 0, user.createdAt, user.updatedAt)
    .run();
  return user;
}
