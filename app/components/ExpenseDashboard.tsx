...
function SalespersonOrderDashboard({ orders, payments, customerById }: { orders: Order[]; payments: Payment[]; customerById: (id: string) => Customer | undefined }) {
  ...
  const eligible = useMemo(() => orders.filter((order) => order.status !== "Cancelled"), [orders]);
...
function OrdersPage({ data, openModal, customerById, personById, vendorById, user, editOrder, viewTransactions }: { data: AppData; openModal: (kind: Exclude<ModalKind, null>) => void; customerById: (id: string) => Customer | undefined; personById: (id: string) => Person | undefined; vendorById: (id: string) => Vendor | undefined; user: PublicUser; editOrder: (order: Order) => void; viewTransactions: (order: Order) => void }) {
  ...
  return <div className="section-stack"><PageHead copy={user.role === "supervisor" ? `${visibleOrders.length} active orders with customer, vendor assignment and execution details.` : `${visibleOrders.length} orders connecting sales value with vendors and payment history.`} action={canCreateRecord(user.role, "order") ? <button className="btn btn-primary" onClick={() => openModal("order")}>＋ Create order</button> : undefined} secondary={activeOrders.length && data.vendors.length ? <button className="btn btn-secondary" onClick={() => openModal("orderVendor")}>Assign vendor</button> : undefined} />{user.role === "sales" && <SalespersonOrderDashboard orders={visibleOrders} payments={data.payments} customerById={customerById} />}{activeOrders.length ? <div className="order-grid">{activeOrders.map((order) => {
  ...
  const eligibleExpensePeople = data.persons.filter((person) => isExpenseResponsiblePerson(person, selectedExpenseOrder));
    ...
    {kind === "order" && <>
      {user.role !== "supervisor" && <>
        ...
        <Field label="Supervisor *"><TeamPersonSelect persons={data.persons} name="assignedPersonId" kind="supervisor" defaultValue={editingOrder?.assignedPersonId} /></Field>
      </>}
...
function OrderSelect({ orders, name, value, defaultValue = "", onChange, showContext = false }: { orders: Order[]; name: string; value?: string; defaultValue?: string; onChange?: (orderId: string) => void; showContext?: boolean }) { return <select name={name} required value={value} defaultValue={value === undefined ? defaultValue : undefined} onChange={onChange ? (event) => onChange(event.target.value) : undefined}><option value="">{orders.length ? "Select order" : "Create an order first"}</option>{orders.map((order) => <option value={order.id} key={order.id}>{order.orderNo}{showContext ? ` · ${order.title || order.venue || order.deliveryAddress || "Venue not added"}` : order.title ? ` · ${order.title}` : ""}</option>)}</select>; }
