...
export default function ExpenseDashboard({ initialSection, user }: { initialSection: string; user: PublicUser }) {
  ...
  const totals = useMemo(() => {
    const orderValue = data.orders.reduce((sum, item) => sum + item.contractValue, 0);
  ...
  const openModal = (kind: Exclude<ModalKind, null>) => {
    ...
    setFormError(""); setFile(null); setEditingOrder(null); setEditingPayment(null); setEditingVendorProduct(null); setModal(kind);
  };
  ...
  const uploadDocument = async () => {
    if (!file) return null;
    const allowedTypes = modal === "order"
    ...
    if (!allowedTypes.includes(file.type)) throw new Error(modal === "order" ? "Choose a PDF, XLS, XLSX, or CSV file" : "Choose a PDF, JPG, PNG, or WebP file");
  ...
  const submitRecord = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/records", { method: isOrderEdit || isPaymentEdit || isVendorProductEdit ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: modal, id: editingOrder?.id ?? editingPayment?.id ?? editingVendorProduct?.id, payload }) });
      const body = await response.json(); if (!response.ok) throw new Error(body.error || "Unable to save record");
      setModal(null); setEditingOrder(null); setEditingPayment(null); setEditingVendorProduct(null); setFile(null); setToast(isOrderEdit ? "Order updated successfully" : isPaymentEdit ? "Payment updated successfully" : isVendorProductEdit ? "Vendor product updated successfully" : `${modalTitles[modal]} saved successfully`); await loadData();
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileMenu ? "open" : ""}`}>
      ...
      <div className="sidebar-help"><div className="help-icon">?</div><strong>Need help?</strong><p>Keep every order, payment and expense connected.</p></div>
    ...
    <main className="main-area">
      ...
      <div className="content-area">{loading ? <LoadingScreen /> : <>{user.role === "supervisor" && data.supervisorLinked === false && <div className="form-error supervisor-link-warning">Your supervisor dashboard is not linked to a People record. Ask an administrator to select your team member in Team access.</div>}
    ...
    {modal && <RecordModal kind={modal} data={data} user={user} preferredVendorId={preferredVendorId} editingOrder={editingOrder} editingPayment={editingPayment} editingVendorProduct={editingVendorProduct} file={file} setFile={setFile} error={formError} saving={saving} onClose={() => { setModal(null); setEditingOrder(null); setEditingPayment(null); setEditingVendorProduct(null); setPreferredVendorId(""); }} onSubmit={submitRecord} />}
    {selectedCustomer && <CustomerDrawer customer={selectedCustomer} data={data} user={user} onClose={() => setSelectedCustomer(null)} />}
    {selectedVendor && <VendorDashboard vendor={selectedVendor} data={data} deletingProductId={deletingProductId} onAddProduct={() => addVendorProduct(selectedVendor)} onEditProduct={editVendorProduct} onDeleteProduct={(product) => void deleteVendorProduct(product)} onClose={() => setSelectedVendor(null)} />}
...
function Overview({ data, totals, openModal, customerById, orderById, user }: { data: AppData; totals: { orderValue: number; received: number; expenses: number; outstanding: number; profit: number }; openModal: (kind: Exclude<ModalKind, null>) => void; customerById: (id: string) => Customer | undefined; orderById: (id: string) => Order | undefined; user: PublicUser }) {
  ...
  return <div className="section-stack">
    <section className="welcome-strip"><div><span className="mini-label">{dateLabel} · {roleLabels[user.role]} dashboard</span><h2>Welcome back, {user.name.split(" ")[0]}.</h2><p>You have <strong>{data.orders.filter((item) => item.status !== "Completed" && item.status !== "Cancelled").length} active orders</strong> visible in your workspace.</p></div><div className="quick-actions">{canCreateRecord(user.role, "customer") && <button onClick={() => openModal("customer")}><span>◎</span><b>New customer</b></button>}{canCreateRecord(user.role, "order") && <button onClick={() => openModal("order")}><span>◇</span><b>New order</b></button>}{canCreateRecord(user.role, "expense") && <button onClick={() => openModal("expense")}><span>↗</span><b>Add expense</b></button>}{canCreateRecord(user.role, "payment") && <button onClick={() => openModal("payment")}><span>₹</span><b>Record payment</b></button>}</div></section>
...
function CustomersPage({ customers, customerBalance, openModal, viewCustomer, user }: { customers: Customer[]; customerBalance: (customer: Customer) => number; openModal: (kind: Exclude<ModalKind, null>) => void; viewCustomer: (customer: Customer) => void; user: PublicUser }) {
  return <div className="section-stack"><PageHead copy={`${customers.length} customer profiles with a complete order and payment trail.`} action={user.role !== "supervisor" ? <button className="btn btn-primary" onClick={() => openModal("customer")}>＋ Add customer</button> : undefined} />{customers.length ? <div className="panel table-panel"><div className="data-table customer-table"><div className="table-row table-header"><span>Customer</span><span>Contact</span><span>GSTIN</span><span>Receivable</span><span>Status</span><span /></div>{customers.map((customer) => { const balance = customerBalance(customer); return <div className="table-row" key={customer.id}><div className="entity-cell"><span className="avatar mint">{initials(customerDisplayName(customer))}</span><div><strong>{customerDisplayName(customer)}</strong><small>{customerCompanyName(customer) || "Individual customer"}</small></div></div><div><strong>{customer.phone}</strong><small>{customer.email || "No email"}</small></div><span>{customer.gstin || "Not added"}</span><strong className={balance > 0 ? "negative" : "positive"}>{balance < 0 ? `${fmt(Math.abs(balance))} advance` : fmt(balance)}</strong><Status value={balance > 0 ? "Payment due" : balance < 0 ? "Advance" : "Paid"} /><button className="text-btn" onClick={() => viewCustomer(customer)}>View profile →</button></div>; })}</div></div> : user.role === "supervisor" ? <div className="mini-empty">No customer is linked to your active orders.</div> : <EmptyState title="Add your first customer" copy="Create a profile to connect orders and payments." action="Create customer" onClick={() => openModal("customer")} />}</div>;
}
...
function VendorDashboard({ vendor, data, deletingProductId, onAddProduct, onEditProduct, onDeleteProduct, onClose }: { vendor: Vendor; data: AppData; deletingProductId: string; onAddProduct: () => void; onEditProduct: (product: VendorProduct) => void; onDeleteProduct: (product: VendorProduct) => void; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="customer-drawer transaction-drawer vendor-dashboard" role="dialog" aria-modal="true" aria-label={`${vendor.name} vendor dashboard`}><button className="modal-close" onClick={onClose} aria-label="Close">×</button><div className="transaction-head"><span className="overline">Vendor dashboard</span><h2>{vendor.name}</h2><p>{vendor.contactPerson || "Vendor contact"} · {vendor.phone}</p></div><div className="transaction-summary vendor-summary"><div><span>Catalog products</span><strong>{products.length}</strong></div><div><span>Order assignments</span><strong>{assignments.length}</strong></div><div><span>Tentative cost</span><strong>{fmt(committed)}</strong></div><div><span>Balance to vendor</span><strong className={committed > paid ? "negative" : "positive"}>{fmt(Math.max(0, committed - paid))}</strong></div></div><div className="drawer-section"><div className="drawer-section-head"><h3>Product & rental catalog</h3><button type="button" className="btn btn-primary btn-small" onClick={onAddProduct}>＋ Add product</button></div><div className="vendor-product-list">{products.map((product) => { const productType = catalogProductType(product.productType); return <div className="vendor-product-row" key={product.id}><div className="grow"><strong>{product.name}</strong><span>{productType} · {product.pricingBasis}</span></div><strong>{fmt(product.rentalCharge)}</strong><div className="vendor-product-actions"><button type="button" className="text-btn" aria-label={`Edit product ${product.name}`} onClick={() => onEditProduct(product)}>Edit product</button><button type="button" className="text-btn danger-text" aria-label={`Delete product ${product.name}`} disabled={deletingProductId === product.id} onClick={() => onDeleteProduct(product)}>{deletingProductId === product.id ? "Deleting…" : "Delete product"}</button></div></div>; })}{!products.length && <div className="mini-empty">Add the first product, choose quantity, length or area pricing, then set its per-day or per-event rate.</div>}</div></div><div className="drawer-section"><h3>Recent order assignments</h3><div className="transaction-list">{assignments.slice(0, 8).map((assignment) => { const order = data.orders.find((item) => item.id === assignment.orderId); const productType = catalogProductType(assignment.productType); const measurement = assignment.measurement || assignment.quantity || 1; return <div className="transaction-item" key={assignment.id}><span className="activity-icon out">◇</span><div className="grow"><strong>{assignment.productName}</strong><span>{order?.orderNo || "Order"} · {measurementLabel(productType)} {measurement}{assignment.pricingBasis === "Per day" ? ` · ${assignment.rentalDays || 1} day(s)` : " · per event"}</span><small>{shortDate(assignment.createdAt)}</small></div><strong>{fmt(assignment.amount)}</strong></div>; })}{!assignments.length && <div className="mini-empty">Products assigned to orders will appear here.</div>}</div></div></aside></div>;
}
...
function OrderTransactionHistory({ order, data, customerById, personById, vendorById, user, onClose }: { order: Order; data: AppData; customerById: (id: string) => Customer | undefined; personById: (id: string) => Person | undefined; vendorById: (id: string) => Vendor | undefined; user: PublicUser; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="customer-drawer transaction-drawer" role="dialog" aria-modal="true" aria-label={`Transactions for ${order.orderNo}`}><button className="modal-close" onClick={onClose} aria-label="Close">×</button><div className="transaction-head"><span className="order-no">{order.orderNo}</span><h2>{orderDisplayTitle(order)}</h2><p>{customerDisplayName(customerById(order.customerId))} · {shortDate(order.eventDate)}</p></div><div className="transaction-summary"><div><span>Received</span><strong className="positive">{fmt(received)}</strong></div><div><span>Vendor paid</span><strong className="negative">{fmt(vendorPaid)}</strong></div><div><span>Expenses</span><strong>{fmt(expenseTotal)}</strong></div><div><span>Net cash</span><strong className={received - vendorPaid >= 0 ? "positive" : "negative"}>{fmt(received - vendorPaid)}</strong></div></div><div className="drawer-section"><h3>Complete order history</h3><div className="transaction-list">{transactions.map((item) => <div className="transaction-item" key={item.id}><span className={`activity-icon ${item.amount >= 0 ? "in" : "out"}`}>{item.amount >= 0 ? "↓" : "↑"}</span><div className="grow"><strong>{item.title}</strong><span>{item.type} · {item.meta}</span><small>{shortDate(item.date)}</small></div><strong className={item.amount >= 0 ? "positive" : "negative"}>{user.role === "supervisor" && item.type === "Vendor assigned" ? "Price hidden" : <>{item.amount >= 0 ? "+" : "−"}{fmt(Math.abs(item.amount))}</>}</strong></div>)}{!transactions.length && <div className="mini-empty">No vendor assignments, expenses or payments have been recorded for this order.</div>}</div></div></aside></div>;
}

function RecordModal({ kind, data, user, preferredVendorId, editingOrder, editingPayment, editingVendorProduct, file, setFile, error, saving, onClose, onSubmit }: { kind: Exclude<ModalKind, null>; data: AppData; user: PublicUser; preferredVendorId: string; editingOrder: Order | null; editingPayment: Payment | null; editingVendorProduct: VendorProduct | null; file: File | null; setFile: (file: File | null) => void; error: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const eligiblePaymentOrders = paymentDirection === "Received"
    ? data.orders.filter((order) => paymentCustomerId && order.customerId === paymentCustomerId)
    : data.orders.filter((order) => paymentVendorId && data.orderVendors.some((assignment) => assignment.orderId === order.id && assignment.vendorId === paymentVendorId));
  ...
  const eligibleExpensePeople = data.persons.filter((person) => isExpenseResponsiblePerson(person, selectedExpenseOrder));
    ...
    {kind === "customer" && <><Field label="Customer name *"><input name="name" required placeholder="e.g. Rohan Mehta" /></Field><Field label="Company name (optional)"><input name="businessName" placeholder="e.g. Quest Strategy" /></Field><Field label="Phone number *"><input name="phone" required inputMode="tel" placeholder="+91 98xxxxxx" /></Field><Field label="Email"><input name="email" type="email" placeholder="accounts@company.com" /></Field><Field label="GSTIN"><input name="gstin" placeholder="27ABCDE1234F1Z5" /></Field><Field label="Opening balance"><input name="openingBalance" type="number" defaultValue="0" /></Field><Field label="Billing address" wide><textarea name="address" rows={3} placeholder="Full billing address" /></Field></>}
    ...
    {kind === "order" && <>
      {user.role !== "supervisor" && <>
        ...
        {user.role === "sales"
          ? <input type="hidden" name="salespersonId" value={user.personId} />
          : <Field label="Salesperson *"><TeamPersonSelect persons={data.persons} teamAssignments={data.teamAssignments} name="salespersonId" kind="salesperson" defaultValue={editingOrder?.salespersonId} /></Field>}
        <Field label="Supervisor *"><TeamPersonSelect persons={data.persons} teamAssignments={data.teamAssignments} name="assignedPersonId" kind="supervisor" defaultValue={editingOrder?.assignedPersonId} /></Field>
      </>}
      ...
      <div className="field field-wide manual-order-choice">
        ...
        <small>When selected, delivery/pickup addresses and contact details become optional. Delivery and return dates remain required.</small>
      </div>
      ...
      {user.role === "sales" && !editingOrder && <>
        <FileField file={file} setFile={setFile} label="Attach product sheet or quotation (optional)" kind="order" />
