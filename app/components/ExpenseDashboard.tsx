...
export default function ExpenseDashboard({ initialSection, user }: { initialSection: string; user: PublicUser }) {
  ...
  const openModal = (kind: Exclude<ModalKind, null>) => {
    ...
    setFormError(""); setFile(null); setEditingOrder(null); setEditingPayment(null); setModal(kind);
  };
  ...
  const submitRecord = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/records", { method: isOrderEdit || isPaymentEdit ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: modal, id: editingOrder?.id ?? editingPayment?.id, payload }) });
      const body = await response.json(); if (!response.ok) throw new Error(body.error || "Unable to save record");
      setModal(null); setEditingOrder(null); setEditingPayment(null); setFile(null); setToast(isOrderEdit ? "Order updated successfully" : isPaymentEdit ? "Payment updated successfully" : `${modalTitles[modal]} saved successfully`); await loadData();
  ...
  return <div className="app-shell">
    ...
    {modal && <RecordModal kind={modal} data={data} user={user} preferredVendorId={preferredVendorId} editingOrder={editingOrder} editingPayment={editingPayment} file={file} setFile={setFile} error={formError} saving={saving} onClose={() => { setModal(null); setEditingOrder(null); setEditingPayment(null); setPreferredVendorId(""); }} onSubmit={submitRecord} />}
...
function RecordModal({ kind, data, user, preferredVendorId, editingOrder, editingPayment, file, setFile, error, saving, onClose, onSubmit }: { kind: Exclude<ModalKind, null>; data: AppData; user: PublicUser; preferredVendorId: string; editingOrder: Order | null; editingPayment: Payment | null; file: File | null; setFile: (file: File | null) => void; error: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  </div>{kind === "order" && (!data.customers.length || !data.persons.length) && <div className="form-hint">Add at least one customer and one person before saving this record.</div>}{kind === "order" && user.role !== "supervisor" && <div className="form-hint">Salesperson and supervisor choices come from People. Their email addresses must match active Sales person and Supervisor accounts in Team access.</div>}{kind === "expense" && (!data.orders.length || !data.persons.length) && <div className="form-hint">Create an order and add a person before recording its expense.</div>}{kind === "payment" && <div className="form-hint">Choose the customer for money received or the vendor for money paid, then select the related order or orders. Accountants can allocate one new payment across several orders. A vendor must be assigned to the selected order before its payout can be recorded.</div>}{error && <div className="form-error" role="alert">! {error}</div>}<div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={onClose}>Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Saving…" : kind === "order" && editingOrder ? "Update order" : kind === "payment" && editingPayment ? "Update payment" : `Save ${kind}`}</button></div></form></div></div>;
}
