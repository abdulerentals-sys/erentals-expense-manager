...
function ExpensesPage({ data, user, openModal }: { data: AppData; user: PublicUser; openModal: (kind: Exclude<ModalKind, null>) => void }) {
  ...
  const updateSupervisorExpense = async (expense: SupervisorExpenseRow, action: "approve" | "disapprove") => {
  ...
  const reimburseSupervisorExpense = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const form = new FormData(event.currentTarget);
    const payload: Record<string, unknown> = { action: "reimburse", expenseId: reimbursementExpense.id, amount, paymentAccountId: String(form.get("paymentAccountId") || ""), method: String(form.get("method") || ""), reference: String(form.get("reference") || "") };
    setWorkflowWorking(`reimburse:${reimbursementExpense.id}`);
    try {
      ...
      setReimbursementExpense(null); setReimbursementFile(null); setWorkflowToast("Reimbursement recorded in payment history");
      await loadWorkflow();
    } catch (error) {
  ...
  return <div className="section-stack expense-dashboard"><PageHead copy={`${filteredExpenses.length} expense records · ${fmt(total)} for ${dateRange.label.toLowerCase()}.`} action={<button className="btn btn-primary" onClick={() => openModal("expense")}>＋ Add expense</button>} secondary={<button className="btn btn-secondary" onClick={exportFilteredExpenses}>⇩ Export Excel</button>} />
    ...
    <section className="panel supervisor-reimbursement-panel" aria-label="Supervisor expense reimbursement workflow">
      ...
      <div className="table-wrap"><table className="expense-workflow-table reimbursement-history-table"><thead><tr><th>Date</th><th>Order</th><th>Supervisor</th><th>Account</th><th>Amount</th><th>Reference</th><th>Receipt</th><th>Notes</th></tr></thead><tbody>{visibleWorkflowPayments.map((payment) => <tr key={payment.id}><td>{shortDate(payment.paymentDate)}</td><td>{orderById(payment.orderId)?.orderNo || payment.orderId || "—"}</td><td>{personById(payment.personId)?.name || workflow.supervisorName || "—"}</td><td>{payment.paymentAccountName || "Legacy / not set"}</td><td className="positive">{fmt(payment.amount)}</td><td>{payment.reference || "—"}</td><td>{payment.receiptKey ? <a className="file-link" href={`/api/upload?key=${encodeURIComponent(payment.receiptKey)}`} target="_blank" rel="noreferrer">▤ View</a> : "—"}</td><td>{payment.notes || "Supervisor reimbursement"}</td></tr>)}</tbody></table>{!visibleWorkflowPayments.length && <div className="mini-empty">No reimbursement payment has been recorded yet.</div>}</div>
    </section>
