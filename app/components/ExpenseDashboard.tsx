...
import { expenseFundingSourceLabel, type ExpenseFundingSource, type PaymentAccountRecord } from "../payment-accounts";
...
function ExpensesPage({ data, user, openModal }: { data: AppData; user: PublicUser; openModal: (kind: Exclude<ModalKind, null>) => void }) {
  ...
  const exportFilteredExpenses = () => {
    ...
    const detailRows: SpreadsheetValue[][] = [["Date", "Order ID", "Order title", "Responsible person", "Role", "Category", "Description", "Payment mode", "Funding source", "Account", "Amount"], ...filteredExpenses.map((expense) => {
      const order = orderById(expense.orderId);
      const person = personById(expense.personId);
      return [expense.expenseDate, order?.orderNo || "", order ? orderDisplayTitle(order) : "", person?.name || "", person?.role || "", expense.category, expense.description, expense.paymentMode, expenseFundingSourceLabel(expense.fundingSource), expense.paymentAccountName || "", expense.amount];
    })];
