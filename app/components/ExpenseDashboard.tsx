...
function AdminOperationsDashboard({ data, customerById }: { data: AppData; customerById: (id: string) => Customer | undefined }) {
  ...
  const eligibleSalespeople = useMemo(() => data.persons.filter((person) => isOrderTeamPerson(person, "salesperson", data.teamAssignments)), [data.persons, data.teamAssignments]);
...
function OrdersPage({ data, openModal, customerById, personById, vendorById, user, editOrder, viewTransactions }: { data: AppData; openModal: (kind: Exclude<ModalKind, null>) => void; customerById: (id: string) => Customer | undefined; personById: (id: string) => Person | undefined; vendorById: (id: string) => Vendor | undefined; user: PublicUser; editOrder: (order: Order) => void; viewTransactions: (order: Order) => void }) {
  return <div className="section-stack"><PageHead copy={user.role === "supervisor" ? `${data.orders.length} active orders with customer, vendor assignment and execution details.` : `${data.orders.length} orders connecting sales value with vendors and payment history.`} action={canCreateRecord(user.role, "order") ? <button className="btn btn-primary" onClick={() => openModal("order")}>＋ Create order</button> : undefined} secondary={data.orders.length && data.vendors.length ? <button className="btn btn-secondary" onClick={() => openModal("orderVendor")}>Assign vendor</button> : undefined} />{data.orders.length ? <div className="order-grid">{data.orders.map((order) => {
  ...
  })}</div> : <EmptyState title="Create your first order" copy={user.role === "supervisor" ? "Assigned active orders will appear here." : "An order connects the customer, execution lead, vendors, payments and expenses."} action="Create order" onClick={() => openModal("order")} />}</div>;
}
