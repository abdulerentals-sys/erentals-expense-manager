...
type Payment = { id: string; invoiceId: string; customerId: string; direction: string; amount: number; paymentDate: string; method: string; reference: string; notes: string; createdAt: string };
...
export default function ExpenseDashboard({ initialSection }: { initialSection: string }) {
  ...
  const invoiceById = useCallback((id: string) => data.invoices.find((item) => item.id === id), [data.invoices]);
  ...
  const submitRecord = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/records", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: modal, payload }) });
      const body = await response.json(); if (!response.ok) throw new Error(body.error || "Unable to save record");
      setModal(null); setFile(null); setToast(`${modalTitles[modal]} saved successfully`); await loadData();
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <div className="content-area">{loading ? <LoadingScreen /> : <>
        ...
        {initialSection === "orders" && <OrdersPage data={data} openModal={openModal} customerById={customerById} personById={personById} />}
        ...
        {initialSection === "payments" && <PaymentsPage payments={data.payments} customerById={customerById} invoiceById={invoiceById} openModal={openModal} />}
    ...
    {modal && <RecordModal kind={modal} data={data} file={file} setFile={setFile} error={formError} saving={saving} onClose={() => setModal(null)} onSubmit={submitRecord} />}
...
function OrdersPage({ data, openModal, customerById, personById }: { data: AppData; openModal: (kind: Exclude<ModalKind, null>) => void; customerById: (id: string) => Customer | undefined; personById: (id: string) => Person | undefined }) {
  return <div className="section-stack"><PageHead copy={`${data.orders.length} orders connecting sales value with execution cost.`} action={<button className="btn btn-primary" onClick={() => openModal("order")}>＋ Create order</button>} />{data.orders.length ? <div className="order-grid">{data.orders.map((order) => { const cost = data.expenses.filter((item) => item.orderId === order.id).reduce((sum, item) => sum + item.amount, 0); const margin = order.contractValue - cost; const progress = Math.min(100, order.contractValue ? (cost / order.contractValue) * 100 : 0); return <article className="order-card" key={order.id}><div className="order-top"><span className="order-no">{order.orderNo}</span><Status value={order.status} /></div><h3>{order.title}</h3><p>{customerById(order.customerId)?.businessName || "Customer"}</p><div className="order-meta"><span>⌖ {order.venue || "Venue not added"}</span><span>◷ {shortDate(order.eventDate)}</span></div><div className="assigned-person"><span className="avatar tiny">{initials(personById(order.assignedPersonId)?.name || "NA")}</span><div><small>Execution lead</small><strong>{personById(order.assignedPersonId)?.name || "Unassigned"}</strong></div></div><div className="budget-line"><span>Execution cost</span><span>{fmt(cost)} of {fmt(order.contractValue)}</span></div><div className="progress-track"><i style={{ width: `${progress}%` }} /></div><div className="order-values"><div><span>Order value</span><strong>{fmt(order.contractValue)}</strong></div><div><span>Gross margin</span><strong className={margin >= 0 ? "positive" : "negative"}>{fmt(margin)}</strong></div></div></article>; })}</div> : <EmptyState title="Create your first order" copy="An order connects the customer, execution lead, invoice and every expense." action="Create order" onClick={() => openModal("order")} />}</div>;
}
...
function PaymentsPage({ payments, customerById, invoiceById, openModal }: { payments: Payment[]; customerById: (id: string) => Customer | undefined; invoiceById: (id: string) => Invoice | undefined; openModal: (kind: Exclude<ModalKind, null>) => void }) {
  return <div className="section-stack"><PageHead copy="A simple cash ledger for incoming and outgoing payments." action={<button className="btn btn-primary" onClick={() => openModal("payment")}>＋ Record payment</button>} />{payments.length ? <div className="panel table-panel"><div className="data-table payment-table"><div className="table-row table-header"><span>Date</span><span>Type</span><span>Party / invoice</span><span>Method</span><span>Reference</span><span>Amount</span></div>{payments.map((payment) => <div className="table-row" key={payment.id}><strong>{shortDate(payment.paymentDate)}</strong><Status value={payment.direction === "Paid" ? "Paid out" : payment.direction} /><div><strong>{customerById(payment.customerId)?.businessName || payment.notes || "General payment"}</strong><small>{invoiceById(payment.invoiceId)?.invoiceNo || "Not linked to invoice"}</small></div><span>{payment.method}</span><span>{payment.reference || "—"}</span><strong className={payment.direction === "Received" ? "positive" : "negative"}>{payment.direction === "Received" ? "+" : "−"}{fmt(payment.amount)}</strong></div>)}</div></div> : <EmptyState title="Start your payment ledger" copy="Record customer receipts or money paid out from the business." action="Record payment" onClick={() => openModal("payment")} />}</div>;
}
...
function RecordModal({ kind, data, file, setFile, error, saving, onClose, onSubmit }: { kind: Exclude<ModalKind, null>; data: AppData; file: File | null; setFile: (file: File | null) => void; error: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const nextInvoice = `INV-${new Date().getFullYear()}-${String(data.invoices.length + 1).padStart(3, "0")}`; const nextOrder = `ORD-${String(data.orders.length + 1).padStart(4, "0")}`; const nextExpense = `EXP-${String(data.expenses.length + 1).padStart(4, "0")}`; const customerOrders = data.orders.filter((order) => order.customerId === invoiceCustomerId); const payableInvoices = data.invoices.filter((item) => item.status !== "Paid" && (!paymentCustomerId || item.customerId === paymentCustomerId));
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="record-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div className="modal-head"><div><span className="overline">eKhata workspace</span><h2 id="modal-title">{modalTitles[kind]}</h2><p>Fields marked with * are required.</p></div><button className="modal-close" onClick={onClose} aria-label="Close">×</button></div><form onSubmit={onSubmit}><div className="form-grid">
    ...
    {kind === "order" && <><Field label="Order number *"><input name="orderNo" required defaultValue={nextOrder} /></Field><Field label="Order title *"><input name="title" required placeholder="e.g. Corporate annual meet" /></Field><Field label="Customer *"><CustomerSelect customers={data.customers} name="customerId" /></Field><Field label="Execution lead *"><PersonSelect persons={data.persons} name="assignedPersonId" /></Field><Field label="Venue"><input name="venue" placeholder="Event venue or site" /></Field><Field label="Event / delivery date *"><input name="eventDate" required type="date" defaultValue={today()} /></Field><Field label="Order value *"><input name="contractValue" required type="number" min="1" placeholder="0" /></Field><Field label="Status"><select name="status"><option>Planned</option><option>In progress</option><option>Completed</option><option>Cancelled</option></select></Field></>}
    ...
    {kind === "payment" && <><Field label="Payment type *"><select name="direction" required value={paymentDirection} onChange={(event) => { const value = event.target.value; setPaymentDirection(value); if (value === "Paid") setPaymentInvoiceId(""); }}><option>Received</option><option>Paid</option></select></Field><Field label="Amount *"><input name="amount" required type="number" min="1" placeholder="0" /></Field><Field label={paymentDirection === "Received" ? "Customer *" : "Customer (optional)"}><select name="customerId" required={paymentDirection === "Received" && !paymentInvoiceId} value={paymentCustomerId} onChange={(event) => { setPaymentCustomerId(event.target.value); setPaymentInvoiceId(""); }}><option value="">{data.customers.length ? "Select customer" : "Add a customer first"}</option>{data.customers.map((customer) => <option value={customer.id} key={customer.id}>{customer.businessName}</option>)}</select></Field><Field label="Invoice"><select name="invoiceId" value={paymentInvoiceId} disabled={paymentDirection === "Paid"} onChange={(event) => { const value = event.target.value; setPaymentInvoiceId(value); const invoice = data.invoices.find((item) => item.id === value); if (invoice) setPaymentCustomerId(invoice.customerId); }}><option value="">{paymentDirection === "Paid" ? "Outgoing payments cannot use an invoice" : "Not linked to invoice"}</option>{payableInvoices.map((item) => <option key={item.id} value={item.id}>{item.invoiceNo} · {fmt(item.total - item.paidAmount)} due</option>)}</select></Field><Field label="Payment date *"><input name="paymentDate" required type="date" defaultValue={today()} /></Field><Field label="Method *"><select name="method" required><option>Bank transfer</option><option>UPI</option><option>Cash</option><option>Cheque</option><option>Credit card</option></select></Field><Field label="Reference"><input name="reference" placeholder="UTR, cheque no. or reference" /></Field><Field label="Notes"><input name="notes" placeholder="Short payment note" /></Field></>}
  </div>{((kind === "order" || kind === "invoice") && (!data.customers.length || !data.persons.length)) && <div className="form-hint">Add at least one customer and one person before saving this record.</div>}{kind === "expense" && (!data.orders.length || !data.persons.length) && <div className="form-hint">Create an order and add a person before recording its expense.</div>}{kind === "payment" && <div className="form-hint">For money received, choose a customer; selecting an invoice automatically uses that invoice&apos;s customer.</div>}{error && <div className="form-error" role="alert">! {error}</div>}<div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={onClose}>Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Saving…" : `Save ${kind}`}</button></div></form></div></div>;
}
...
function CustomerSelect({ customers, name, optional }: { customers: Customer[]; name: string; optional?: boolean }) { return <select name={name} required={!optional} defaultValue=""><option value="">{customers.length ? "Select customer" : "Add a customer first"}</option>{customers.map((customer) => <option value={customer.id} key={customer.id}>{customer.businessName}</option>)}</select>; }
function PersonSelect({ persons, name }: { persons: Person[]; name: string }) { return <select name={name} required defaultValue=""><option value="">{persons.length ? "Select person" : "Add a person first"}</option>{persons.map((person) => <option value={person.id} key={person.id}>{person.name} · {person.role}</option>)}</select>; }
