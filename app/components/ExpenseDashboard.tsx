...
type Payment = { id: string; orderId: string; personId: string; vendorId: string; customerId: string; direction: string; amount: number; paymentDate: string; method: string; reference: string; notes: string; createdAt: string };
type AppData = { customers: Customer[]; historyCustomers: Customer[]; persons: Person[]; vendors: Vendor[]; vendorProducts: VendorProduct[]; orders: Order[]; historyOrders: Order[]; orderVendors: OrderVendor[]; expenses: Expense[]; payments: Payment[]; supervisorLinked?: boolean };
type ModalKind = "customer" | "person" | "vendor" | "vendorProduct" | "order" | "orderVendor" | "expense" | "payment" | null;

const emptyData: AppData = { customers: [], historyCustomers: [], persons: [], vendors: [], vendorProducts: [], orders: [], historyOrders: [], orderVendors: [], expenses: [], payments: [] };
const navItems = [
...
function RecordModal({ kind, data, user, preferredVendorId, editingOrder, editingPayment, editingVendorProduct, file, setFile, error, saving, onClose, onSubmit }: { kind: Exclude<ModalKind, null>; data: AppData; user: PublicUser; preferredVendorId: string; editingOrder: Order | null; editingPayment: Payment | null; editingVendorProduct: VendorProduct | null; file: File | null; setFile: (file: File | null) => void; error: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const [paymentOrderId, setPaymentOrderId] = useState(editingPayment?.orderId ?? "");
    ...
    {kind === "payment" && <>
      ...
      {paymentDirection === "Received" && <Field label="Customer *">
        <select name="customerId" required value={paymentCustomerId} onChange={(event) => {
          ...
          {data.customers.map((customer) => <option value={customer.id} key={customer.id}>{customer.businessName}</option>)}
        </select>
      </Field>}
          ...
          {eligiblePaymentOrders.map((order) => {
            ...
            return <div className={`allocation-row ${selected ? "selected" : ""}`} key={order.id}>
              <label>
                ...
                <span><strong>{order.orderNo}</strong><small>{order.title ? `${order.title} · ` : ""}{data.customers.find((customer) => customer.id === order.customerId)?.businessName || "Customer"}</small></span>
              </label>
  ...
  </div>{kind === "order" && (!data.customers.length || !data.persons.length) && <div className="form-hint">Add at least one customer and one person before saving this record.</div>}{kind === "order" && user.role !== "supervisor" && <div className="form-hint">Salesperson and supervisor choices come directly from People. Link their dashboard accounts separately in Team access.</div>}{kind === "expense" && (!data.orders.length || !data.persons.length) && <div className="form-hint">Create an order and add a person before recording its expense.</div>}{kind === "payment" && <div className="form-hint">Choose the customer for money received or the vendor for money paid, then select the related order or orders. Accountants can allocate one new payment across several orders. A vendor must be assigned to the selected order before its payout can be recorded.</div>}{error && <div className="form-error" role="alert">! {error}</div>}<div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={onClose}>Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Saving…" : kind === "order" && editingOrder ? "Update order" : kind === "payment" && editingPayment ? "Update payment" : kind === "vendorProduct" && editingVendorProduct ? "Update vendor product" : `Save ${kind}`}</button></div></form></div></div>;
}
