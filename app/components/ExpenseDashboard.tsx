...
type AppData = { customers: Customer[]; persons: Person[]; vendors: Vendor[]; orders: Order[]; orderVendors: OrderVendor[]; invoices: Invoice[]; expenses: Expense[]; payments: Payment[] };
type ModalKind = "customer" | "person" | "vendor" | "order" | "orderVendor" | "invoice" | "expense" | "payment" | null;

const emptyData: AppData = { customers: [], persons: [], vendors: [], orders: [], orderVendors: [], invoices: [], expenses: [], payments: [] };
const navItems = [
...
const modalTitles: Record<Exclude<ModalKind, null>, string> = {
  customer: "Create customer profile", person: "Add team member", vendor: "Add vendor record", order: "Create a new order", orderVendor: "Assign vendor to order", invoice: "Create invoice", expense: "Add order expense", payment: "Record payment",
};
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <div className="content-area">{loading ? <LoadingScreen /> : <>
        ...
        {initialSection === "vendors" && <VendorsPage vendors={data.vendors} orderVendors={data.orderVendors} payments={data.payments} expenses={data.expenses} openModal={openModal} />}
    ...
    {selectedOrder && <OrderTransactionHistory order={selectedOrder} data={data} customerById={customerById} personById={personById} vendorById={vendorById} onClose={() => setSelectedOrder(null)} />}
...
function VendorsPage({ vendors, orderVendors, payments, expenses, openModal }: { vendors: Vendor[]; orderVendors: OrderVendor[]; payments: Payment[]; expenses: Expense[]; openModal: (kind: Exclude<ModalKind, null>) => void }) {
  return <div className="section-stack"><PageHead copy={`${vendors.length} vendor records connected to orders, expenses and payouts.`} action={<button className="btn btn-primary" onClick={() => openModal("vendor")}>＋ Add vendor</button>} secondary={vendors.length ? <button className="btn btn-secondary" onClick={() => openModal("orderVendor")}>Assign to order</button> : undefined} />{vendors.length ? <div className="person-grid">{vendors.map((vendor) => { const assignments = orderVendors.filter((item) => item.vendorId === vendor.id); const committed = assignments.reduce((sum, item) => sum + item.amount, 0); const spent = expenses.filter((item) => item.vendorId === vendor.id).reduce((sum, item) => sum + item.amount, 0); const paid = payments.filter((item) => item.vendorId === vendor.id && item.direction === "Paid").reduce((sum, item) => sum + item.amount, 0); return <article className="person-card" key={vendor.id}><div className="person-card-head"><span className="avatar large">{initials(vendor.name)}</span><Status value={vendor.status} /></div><h3>{vendor.name}</h3><p>{vendor.contactPerson || "Vendor"}</p><div className="contact-lines"><span>☎ {vendor.phone}</span><span>✉ {vendor.email || "No email added"}</span><span>GSTIN: {vendor.gstin || "Not added"}</span></div><div className="person-stats"><div><span>Assignments</span><strong>{assignments.length}</strong></div><div><span>Committed</span><strong>{fmt(committed)}</strong></div><div><span>Expenses</span><strong>{fmt(spent)}</strong></div><div><span>Paid</span><strong>{fmt(paid)}</strong></div></div><div className="person-footer"><span>Preferred payment</span><strong>{vendor.paymentMode}</strong></div></article>; })}</div> : <EmptyState title="Add your first vendor" copy="Create a vendor record, then assign products and amounts to orders." action="Add vendor" onClick={() => openModal("vendor")} />}</div>;
}
...
function OrdersPage({ data, openModal, customerById, personById, vendorById, user, editOrder, viewTransactions }: { data: AppData; openModal: (kind: Exclude<ModalKind, null>) => void; customerById: (id: string) => Customer | undefined; personById: (id: string) => Person | undefined; vendorById: (id: string) => Vendor | undefined; user: PublicUser; editOrder: (order: Order) => void; viewTransactions: (order: Order) => void }) {
  return <div className="section-stack"><PageHead copy={`${data.orders.length} orders connecting sales value with vendors and payment history.`} action={<button className="btn btn-primary" onClick={() => openModal("order")}>＋ Create order</button>} secondary={data.orders.length && data.vendors.length ? <button className="btn btn-secondary" onClick={() => openModal("orderVendor")}>Assign vendor</button> : undefined} />{data.orders.length ? <div className="order-grid">{data.orders.map((order) => {
...
function OrderTransactionHistory({ order, data, customerById, personById, vendorById, onClose }: { order: Order; data: AppData; customerById: (id: string) => Customer | undefined; personById: (id: string) => Person | undefined; vendorById: (id: string) => Vendor | undefined; onClose: () => void }) {
  const orderPayments = data.payments.filter((item) => item.orderId === order.id);
  const orderExpenses = data.expenses.filter((item) => item.orderId === order.id);
  const orderInvoices = data.invoices.filter((item) => item.orderId === order.id);
...
function RecordModal({ kind, data, user, editingOrder, file, setFile, error, saving, onClose, onSubmit }: { kind: Exclude<ModalKind, null>; data: AppData; user: PublicUser; editingOrder: Order | null; file: File | null; setFile: (file: File | null) => void; error: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="record-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div className="modal-head"><div><span className="overline">eKhata workspace</span><h2 id="modal-title">{kind === "order" && editingOrder ? "Edit order" : modalTitles[kind]}</h2><p>Fields marked with * are required.</p></div><button className="modal-close" onClick={onClose} aria-label="Close">×</button></div><form onSubmit={onSubmit}><div className="form-grid">
    ...
    {kind === "orderVendor" && <><Field label="Order *"><OrderSelect orders={data.orders} name="orderId" /></Field><Field label="Vendor *"><VendorSelect vendors={data.vendors} name="vendorId" /></Field><Field label="Product / service *"><input name="productName" required placeholder="e.g. LED wall rental" /></Field><Field label="Amount *"><input name="amount" required type="number" min="1" placeholder="0" /></Field><Field label="Notes" wide><textarea name="notes" rows={2} placeholder="Quantity, specifications or terms" /></Field></>}
...
function VendorSelect({ vendors, name, optional, defaultValue = "" }: { vendors: Vendor[]; name: string; optional?: boolean; defaultValue?: string }) { return <select name={name} required={!optional} defaultValue={defaultValue}><option value="">{vendors.length ? optional ? "No vendor" : "Select vendor" : "Add a vendor first"}</option>{vendors.map((vendor) => <option value={vendor.id} key={vendor.id}>{vendor.name}{vendor.contactPerson ? ` · ${vendor.contactPerson}` : ""}</option>)}</select>; }
