...
export default function ExpenseDashboard({ initialSection, user }: { initialSection: string; user: PublicUser }) {
  ...
  const submitRecord = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/records", { method: isCustomerEdit || isOrderEdit || isPaymentEdit || isVendorProductEdit ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: modal, id: editingCustomer?.id ?? editingOrder?.id ?? editingPayment?.id ?? editingVendorProduct?.id, payload }) });
      const body = await response.json(); if (!response.ok) throw new Error(body.error || "Unable to save record");
      setModal(null); setEditingCustomer(null); setEditingOrder(null); setEditingPayment(null); setEditingVendorProduct(null); setFile(null); setToast(isCustomerEdit ? "Customer updated successfully" : isOrderEdit ? "Order updated successfully" : isPaymentEdit ? "Payment updated successfully" : isVendorProductEdit ? "Vendor product updated successfully" : `${modalTitles[modal]} saved successfully`); await loadData();
  ...
  return <div className="app-shell">
    ...
    {modal && <RecordModal kind={modal} data={data} user={user} preferredVendorId={preferredVendorId} editingCustomer={editingCustomer} editingOrder={editingOrder} editingPayment={editingPayment} editingVendorProduct={editingVendorProduct} file={file} setFile={setFile} error={formError} saving={saving} onClose={() => { setModal(null); setEditingCustomer(null); setEditingOrder(null); setEditingPayment(null); setEditingVendorProduct(null); setPreferredVendorId(""); }} onSubmit={submitRecord} />}
...
function AdminOperationsDashboard({ data, customerById }: { data: AppData; customerById: (id: string) => Customer | undefined }) {
  ...
  const supervisorRows = useMemo(() => eligibleSupervisors.map((person) => {
    const orders = data.orders.filter((order) => isOrderSupervisor(order, person.id) && dateIsInRange(indiaDateKey(order.createdAt), range));
...
function RecordModal({ kind, data, user, preferredVendorId, editingCustomer, editingOrder, editingPayment, editingVendorProduct, file, setFile, error, saving, onClose, onSubmit }: { kind: Exclude<ModalKind, null>; data: AppData; user: PublicUser; preferredVendorId: string; editingCustomer: Customer | null; editingOrder: Order | null; editingPayment: Payment | null; editingVendorProduct: VendorProduct | null; file: File | null; setFile: (file: File | null) => void; error: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const eligibleExpensePeople = data.persons.filter((person) => isExpenseResponsiblePerson(person, selectedExpenseOrder));
		  return <div className="modal-backdrop" role="presentation" onMouseDown={kind === "order" ? undefined : (event) => event.target === event.currentTarget && onClose()}><div className="record-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div className="modal-head"><div><span className="overline">eKhata workspace</span><h2 id="modal-title">{kind === "customer" && editingCustomer ? "Edit customer profile" : kind === "order" && editingOrder ? "Edit order" : kind === "payment" && editingPayment ? "Edit payment" : kind === "vendorProduct" && editingVendorProduct ? "Edit vendor product" : modalTitles[kind]}</h2><p>Fields marked with * are required.</p></div><button type="button" className="modal-close" onClick={onClose} aria-label={kind === "order" ? "Close order form" : "Close form"}>×</button></div><form onSubmit={onSubmit}><div className="form-grid">
