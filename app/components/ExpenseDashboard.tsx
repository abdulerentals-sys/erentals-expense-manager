...
import { expenseNeedsReimbursement, type ExpenseFundingSource, type PaymentAccountRecord } from "../payment-accounts";
...
type Expense = { id: string; expenseNo: string; orderId: string; personId: string; vendorId: string; category: string; vendor: string; description: string; expenseDate: string; amount: number; paymentMode: string; receiptKey: string; receiptName: string; fundingSource?: ExpenseFundingSource; paymentAccountId?: string; paymentAccountName?: string; status?: SupervisorExpenseStatus; reimbursedAmount?: number; createdAt: string };
type ExpenseCategoryRecord = { id: string; name: string; status: string; createdAt: string };
type PaymentAccount = PaymentAccountRecord;
type Payment = { id: string; orderId: string; manualOrderId: string; personId: string; vendorId: string; expenseId?: string; customerId: string; direction: string; amount: number; paymentDate: string; method: string; reference: string; notes: string; paymentAccountId?: string; paymentAccountName?: string; receiptKey?: string; receiptName?: string; createdAt: string };
...
const emptyData: AppData = { customers: [], historyCustomers: [], persons: [], vendors: [], vendorProducts: [], orders: [], historyOrders: [], orderProducts: [], orderVendors: [], expenses: [], expenseCategories: [], paymentAccounts: [], payments: [] };
...
function Status({ value }: { value: string }) {
  const tone = ["Paid", "Active", "Completed", "Received", "Advance", "Approved", "Reimbursed"].includes(value) ? "success" : ["Overdue", "Cancelled", "Archived", "Paid out", "Disapproved", "Rejected"].includes(value) ? "danger" : ["Part paid", "In progress", "Sent", "Payment due", "Pending approval", "Pending request", "Awaiting approval", "Pending reimbursement"].includes(value) ? "warning" : "neutral";
...
export default function ExpenseDashboard({ initialSection, user }: { initialSection: string; user: PublicUser }) {
  ...
  const [updatingPaymentAccountId, setUpdatingPaymentAccountId] = useState("");
  const [deletingPaymentAccountId, setDeletingPaymentAccountId] = useState("");
  ...
  const addPaymentAccount = async (name: string) => {
    setSavingPaymentAccount(true);
    try {
      const response = await fetch("/api/records", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "paymentAccount", payload: { name } }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to add payment account");
      setToast("Payment account added successfully");
      await loadData();
      return true;
    } catch (error) { setToast(error instanceof Error ? error.message : "Unable to add payment account"); return false; }
    finally { setSavingPaymentAccount(false); }
  };

  const updatePaymentAccount = async (account: PaymentAccount, name: string) => {
    setUpdatingPaymentAccountId(account.id);
    try {
      const response = await fetch("/api/records", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "paymentAccount", id: account.id, payload: { name } }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to update payment account");
      setToast("Payment account updated successfully");
      await loadData();
      return true;
    } catch (error) { setToast(error instanceof Error ? error.message : "Unable to update payment account"); return false; }
    finally { setUpdatingPaymentAccountId(""); }
  };

  const deletePaymentAccount = async (account: PaymentAccount) => {
    if (!window.confirm(`Delete ${account.name} from future payment forms? Historical records will keep the account name.`)) return;
    setDeletingPaymentAccountId(account.id);
    try {
      const response = await fetch("/api/records", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "paymentAccount", id: account.id }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to delete payment account");
      setToast("Payment account removed from future forms");
      await loadData();
    } catch (error) { setToast(error instanceof Error ? error.message : "Unable to delete payment account"); }
    finally { setDeletingPaymentAccountId(""); }
  };
  ...
  const submitRecord = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if ((modal === "expense" || modal === "payment") && file) {
        const upload = await uploadDocument();
        if (upload) { payload.receiptKey = upload.key; payload.receiptName = upload.name; }
      }
      if (modal === "order" && file) {
        const upload = await uploadDocument();
  ...
  const exportExpenses = () => {
    const rows = [["Date", "Order", "Person", "Category", "Vendor", "Amount", "Payment mode", "Funding source", "Account"], ...data.expenses.map((expense) => { const order = orderById(expense.orderId); return [expense.expenseDate, order ? orderDisplayTitle(order) : "", personById(expense.personId)?.name ?? "", expense.category, expense.vendor, String(expense.amount), expense.paymentMode, expenseNeedsReimbursement(expense) ? "Reimbursement" : "Company account", expense.paymentAccountName || ""]; })];
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <div className="content-area">{loading ? <LoadingScreen /> : <>{user.role === "supervisor" && data.supervisorLinked === false && <div className="form-error supervisor-link-warning">Add an active People record with the same name as this account and a Supervisor role to receive assigned orders.</div>}
        ...
        {initialSection === "settings" && <SettingsPage customCategories={data.expenseCategories} paymentAccounts={data.paymentAccounts} saving={savingCategory} deletingId={deletingCategoryId} savingAccount={savingPaymentAccount} updatingAccountId={updatingPaymentAccountId} deletingAccountId={deletingPaymentAccountId} onAdd={addExpenseCategory} onDelete={(category) => void deleteExpenseCategory(category)} onAddAccount={addPaymentAccount} onUpdateAccount={updatePaymentAccount} onDeleteAccount={(account) => void deletePaymentAccount(account)} />}
      </>}</div>
    </main>
...
function SettingsPage({ customCategories, paymentAccounts, saving, deletingId, savingAccount, updatingAccountId, deletingAccountId, onAdd, onDelete, onAddAccount, onUpdateAccount, onDeleteAccount }: { customCategories: ExpenseCategoryRecord[]; paymentAccounts: PaymentAccount[]; saving: boolean; deletingId: string; savingAccount: boolean; updatingAccountId: string; deletingAccountId: string; onAdd: (name: string) => Promise<boolean>; onDelete: (category: ExpenseCategoryRecord) => void; onAddAccount: (name: string) => Promise<boolean>; onUpdateAccount: (account: PaymentAccount, name: string) => Promise<boolean>; onDeleteAccount: (account: PaymentAccount) => void }) {
  ...
  const submitAccount = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (await onAddAccount(accountName)) setAccountName("");
  };
  return <div className="section-stack settings-category-manager"><PageHead copy="Administrators can manage expense categories and the company accounts used for expenses, payments and reimbursements. Historical records keep their original account names." /><div className="settings-grid"><article className="panel settings-create-card"><span className="overline">Expense setup</span><h2>Add a new category</h2><p>Custom categories become available immediately in expense forms for every permitted role.</p><form onSubmit={submit}><Field label="Category name *"><input value={name} onChange={(event) => setName(event.target.value)} required maxLength={60} placeholder="e.g. Equipment repair" /></Field><button type="submit" className="btn btn-primary" disabled={saving || !name.trim()}>{saving ? "Adding…" : "＋ Add category"}</button></form></article><article className="panel settings-list-card"><div className="panel-head"><div><span className="overline">Category library</span><h3>Expense categories</h3></div><span className="schedule-count">{EXPENSE_CATEGORIES.length + customCategories.length}</span></div><div className="settings-category-list">{EXPENSE_CATEGORIES.map((category) => <div className="settings-category-row" key={category}><div><strong>{category}</strong><span>Built-in category</span></div><span className="settings-lock">Protected</span></div>)}{customCategories.map((category) => <div className="settings-category-row" key={category.id}><div><strong>{category.name}</strong><span>Custom category</span></div><button type="button" className="btn-icon-danger" disabled={deletingId === category.id} onClick={() => onDelete(category)} aria-label={`Delete ${category.name}`}>{deletingId === category.id ? "…" : "Delete"}</button></div>)}</div></article></div><div className="settings-grid payment-account-settings"><article className="panel settings-create-card"><span className="overline">Payment setup</span><h2>Add a payment account</h2><p>Accounts are available in payment records, company-funded expenses and supervisor reimbursements.</p><form onSubmit={submitAccount}><Field label="Account name *"><input value={accountName} onChange={(event) => setAccountName(event.target.value)} required maxLength={80} placeholder="e.g. Main current account" /></Field><button type="submit" className="btn btn-primary" disabled={savingAccount || !accountName.trim()}>{savingAccount ? "Adding…" : "＋ Add account"}</button></form></article><article className="panel settings-list-card"><div className="panel-head"><div><span className="overline">Account library</span><h3>Payment accounts</h3></div><span className="schedule-count">{paymentAccounts.length}</span></div><div className="settings-category-list">{paymentAccounts.map((account) => <PaymentAccountEditor key={account.id} account={account} updating={updatingAccountId === account.id} deleting={deletingAccountId === account.id} onUpdate={onUpdateAccount} onDelete={onDeleteAccount} />)}{!paymentAccounts.length && <div className="mini-empty">Add an account before recording payments or company-funded expenses.</div>}</div></article></div></div>;
}

function PaymentAccountEditor({ account, updating, deleting, onUpdate, onDelete }: { account: PaymentAccount; updating: boolean; deleting: boolean; onUpdate: (account: PaymentAccount, name: string) => Promise<boolean>; onDelete: (account: PaymentAccount) => void }) {
  const [name, setName] = useState(account.name);
  return <form className="settings-category-row payment-account-row" onSubmit={async (event) => { event.preventDefault(); if (await onUpdate(account, name)) setName(name.trim()); }}><div><input aria-label={`Account name for ${account.name}`} value={name} onChange={(event) => setName(event.target.value)} required maxLength={80} /><span>Available for payment records</span></div><div className="payment-account-actions"><button type="submit" className="text-btn" disabled={updating || !name.trim() || name.trim() === account.name}>{updating ? "Updating…" : "Update account"}</button><button type="button" className="btn-icon-danger" disabled={deleting} onClick={() => onDelete(account)}>{deleting ? "…" : "Delete"}</button></div></form>;
}
...
function ExpensesPage({ data, user, openModal }: { data: AppData; user: PublicUser; openModal: (kind: Exclude<ModalKind, null>) => void }) {
  ...
  const [reimbursementView, setReimbursementView] = useState<"all" | "pending">("all");
  const [reviewExpense, setReviewExpense] = useState<SupervisorExpenseRow | null>(null);
  const [reimbursementExpense, setReimbursementExpense] = useState<SupervisorExpenseRow | null>(null);
  ...
  useEffect(() => { const timer = window.setTimeout(() => void loadWorkflow(), 0); return () => window.clearTimeout(timer); }, [loadWorkflow, data.expenses.length]);
  ...
  const pendingRequestExpenses = workflow.expenses.filter((expense) => expense.status === "Pending approval");
  const visibleWorkflowExpenses = reimbursementView === "pending" ? pendingRequestExpenses : workflowExpenses;
  const pendingApproval = pendingRequestExpenses.reduce((sum, expense) => sum + expense.amount, 0);
  const supervisorSummaries = useMemo(() => {
    ...
    for (const expense of workflowExpenses) {
      ...
      current.approved += ["Approved", "Paid"].includes(expense.status) ? expense.amount : 0;
  ...
  const updateSupervisorExpense = async (expense: SupervisorExpenseRow, action: "approve" | "reject") => {
    if (action === "reject" && !window.confirm("Reject this reimbursement? It will not enter the pending reimbursement total.")) return;
    setWorkflowWorking(`${action}:${expense.id}`);
    try {
      const response = await fetch("/api/expense-approvals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, expenseId: expense.id }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to update reimbursement");
      setWorkflowToast(action === "approve" ? "Reimbursement request accepted" : "Reimbursement request rejected");
      await loadWorkflow();
      setReviewExpense(action === "approve" ? { ...expense, status: "Approved" } : null);
    } catch (error) {
      setWorkflowToast(error instanceof Error ? error.message : "Unable to update reimbursement");
    } finally {
      setWorkflowWorking("");
    }
  };

  const reimburseSupervisorExpense = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!reimbursementExpense) return;
    const amount = reimbursementPending(reimbursementExpense.amount, reimbursementExpense.reimbursedAmount);
    const form = new FormData(event.currentTarget);
    const payload: Record<string, unknown> = { action: "paid", expenseId: reimbursementExpense.id, amount, paymentAccountId: String(form.get("paymentAccountId") || ""), method: String(form.get("method") || ""), reference: String(form.get("reference") || "") };
    setWorkflowWorking(`paid:${reimbursementExpense.id}`);
    try {
      if (reimbursementFile) {
        if (!isSupportedReceiptDocument(reimbursementFile.type)) throw new Error("Choose an image or PDF file");
        if (reimbursementFile.size > 10 * 1024 * 1024) throw new Error("The file must be smaller than 10 MB");
        const uploadForm = new FormData(); uploadForm.append("file", reimbursementFile); uploadForm.append("kind", "payment");
        const uploadResponse = await fetch("/api/upload", { method: "POST", body: uploadForm });
        const uploadBody = await uploadResponse.json();
        if (!uploadResponse.ok) throw new Error(uploadBody.error || "Receipt upload failed");
        payload.receiptKey = uploadBody.key; payload.receiptName = uploadBody.name;
      }
      const response = await fetch("/api/expense-approvals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to record reimbursement");
      setReimbursementExpense(null); setReimbursementFile(null); setWorkflowToast("Reimbursement marked paid and recorded in payment history");
      await loadWorkflow();
    } catch (error) {
      setWorkflowToast(error instanceof Error ? error.message : "Unable to record reimbursement");
    } finally {
      setWorkflowWorking("");
    }
  };
  ...
  const exportFilteredExpenses = () => {
    ...
    const detailRows: SpreadsheetValue[][] = [["Date", "Order ID", "Order title", "Responsible person", "Role", "Category", "Description", "Payment mode", "Funding source", "Account", "Amount"], ...filteredExpenses.map((expense) => {
      const order = orderById(expense.orderId);
      const person = personById(expense.personId);
      return [expense.expenseDate, order?.orderNo || "", order ? orderDisplayTitle(order) : "", person?.name || "", person?.role || "", expense.category, expense.description, expense.paymentMode, expenseNeedsReimbursement(expense) ? "Reimbursement" : "Company account", expense.paymentAccountName || "", expense.amount];
    })];
  ...
  return <div className="section-stack expense-dashboard"><PageHead copy={`${filteredExpenses.length} expense records · ${fmt(total)} for ${dateRange.label.toLowerCase()}.`} action={<button className="btn btn-primary" onClick={() => openModal("expense")}>＋ Add expense</button>} secondary={<button className="btn btn-secondary" onClick={exportFilteredExpenses}>⇩ Export Excel</button>} />
    ...
    <section className="panel supervisor-reimbursement-panel" aria-label="Supervisor expense reimbursement workflow">
      <div className="panel-head"><div><span className="overline">Reimbursements</span><h3>{user.role === "supervisor" ? "My expense status" : "Approval and payment status"}</h3><p>{user.role === "supervisor" ? "Track submitted, approved, rejected, pending and paid expenses." : "Review supervisor and admin reimbursement submissions, then keep every paid claim in payment history."}</p></div>{workflowLoading && <span className="expense-result-count">Loading…</span>}</div>
      {workflowError && <div className="form-error">{workflowError}</div>}
      <div className="reimbursement-metric-grid"><div><span>Total reimbursement expenses</span><strong>{fmt(workflowExpenses.reduce((sum, expense) => sum + expense.amount, 0))}</strong><small>{workflowExpenses.length} submitted · {fmt(approvedSupervisorCost)} approved or paid</small></div>{user.role === "admin" ? <button type="button" className={reimbursementView === "pending" ? "active" : ""} aria-pressed={reimbursementView === "pending"} onClick={() => setReimbursementView("pending")}><span>Pending requests</span><strong>{fmt(pendingApproval)}</strong><small>{pendingRequestExpenses.length} request{pendingRequestExpenses.length === 1 ? "" : "s"} · Click to review</small></button> : <div><span>Pending requests</span><strong>{fmt(pendingApproval)}</strong><small>{pendingRequestExpenses.length} request{pendingRequestExpenses.length === 1 ? "" : "s"}</small></div>}<div><span>Pending reimbursement</span><strong className="negative">{fmt(pendingReimbursement)}</strong><small>Approved and not yet paid</small></div><div><span>Paid</span><strong className="positive">{fmt(reimbursedSupervisorCost)}</strong><small>Moved to payment history</small></div></div>
      <div className="supervisor-reimbursement-list"><div className="reimbursement-subhead"><h4>{user.role === "supervisor" ? "My reimbursement summary" : "Reimbursement by claimant"}</h4><span>{supervisorSummaries.length} claimant{supervisorSummaries.length === 1 ? "" : "s"}</span></div>{supervisorSummaries.map((row) => <button type="button" className="supervisor-reimbursement-row" key={row.id} onClick={() => setExpenseDetail({ kind: "person", id: row.id, title: row.name })}><span className="avatar tiny">{initials(row.name)}</span><span className="grow"><strong>{row.name}</strong><small>{row.count} submitted · {fmt(row.approved)} approved or paid</small></span><span><small>Total</small><strong>{fmt(row.submitted)}</strong></span><span><small>Pending</small><strong className={row.pending ? "negative" : "positive"}>{fmt(row.pending)}</strong></span><span><small>Paid</small><strong className="positive">{fmt(row.reimbursed)}</strong></span></button>)}{!workflowLoading && !supervisorSummaries.length && <div className="mini-empty">No reimbursement expenses match this reporting period.</div>}</div>
      <div className="reimbursement-subhead"><h4>{reimbursementView === "pending" ? "Pending requests" : "Reimbursement entries"}</h4><div className="reimbursement-view-actions"><span>{visibleWorkflowExpenses.length} record{visibleWorkflowExpenses.length === 1 ? "" : "s"}</span>{reimbursementView === "pending" && <button type="button" className="text-btn" onClick={() => setReimbursementView("all")}>View all</button>}</div></div>
      <div className="table-wrap"><table className="expense-workflow-table"><thead><tr><th>Date</th><th>Order</th><th>Claimant / Submitted by</th><th>Expense</th><th>Status</th><th>Payment status</th><th>Pending</th><th>Action</th></tr></thead><tbody>{visibleWorkflowExpenses.map((expense) => { const pendingAmount = expense.status === "Approved" ? reimbursementPending(expense.amount, expense.reimbursedAmount) : 0; const paymentStatus = expense.status === "Rejected" ? "Rejected" : expense.status === "Pending approval" ? "Pending request" : expense.status === "Paid" || pendingAmount === 0 ? "Paid" : "Pending reimbursement"; return <tr key={expense.id}><td>{shortDate(expense.expenseDate)}</td><td><strong>{expense.orderNo}</strong><small>{expense.orderTitle}</small></td><td><strong>{expense.supervisorName}</strong><small>Submitted by {expense.submittedByName || "Legacy supervisor"} · {expense.submittedByRole || "supervisor"}</small></td><td><strong>{fmt(expense.amount)}</strong><small>{expense.category}</small></td><td><Status value={expense.status} /></td><td><Status value={paymentStatus} /></td><td className={pendingAmount ? "negative" : "positive"}>{pendingAmount ? fmt(pendingAmount) : "—"}</td><td className="reimbursement-actions"><button type="button" className="btn btn-secondary btn-small" onClick={() => setReviewExpense(expense)}>{user.role === "admin" && expense.status === "Pending approval" ? "Review request" : "View details"}</button></td></tr>; })}</tbody></table>{!workflowLoading && !visibleWorkflowExpenses.length && <div className="mini-empty">{reimbursementView === "pending" ? "No pending reimbursement requests match this view." : "No reimbursement entries match this view."}</div>}</div>
      ...
      <div className="table-wrap"><table className="expense-workflow-table reimbursement-history-table"><thead><tr><th>Date</th><th>Order</th><th>Claimant</th><th>Account</th><th>Amount</th><th>Reference</th><th>Receipt</th><th>Notes</th></tr></thead><tbody>{visibleWorkflowPayments.map((payment) => <tr key={payment.id}><td>{shortDate(payment.paymentDate)}</td><td>{orderById(payment.orderId)?.orderNo || payment.orderId || "—"}</td><td>{personById(payment.personId)?.name || workflowExpenseById.get(payment.expenseId || "")?.supervisorName || workflow.expenses.find((expense) => expense.personId === payment.personId)?.supervisorName || workflow.supervisorName || "—"}</td><td>{payment.paymentAccountName || "Legacy / not set"}</td><td className="positive">{fmt(payment.amount)}</td><td>{payment.reference || "—"}</td><td>{payment.receiptKey ? <a className="file-link" href={`/api/upload?key=${encodeURIComponent(payment.receiptKey)}`} target="_blank" rel="noreferrer">▤ View</a> : "—"}</td><td>{payment.notes || "Expense reimbursement"}</td></tr>)}</tbody></table>{!visibleWorkflowPayments.length && <div className="mini-empty">No reimbursement payment has been recorded yet.</div>}</div>
    </section>
    ...
    {expenseDetail && <div className="modal-backdrop" role="presentation"><aside className="customer-drawer expense-detail-drawer" role="dialog" aria-modal="true" aria-label={`Expense details for ${expenseDetail.title}`}><button type="button" className="modal-close" onClick={() => setExpenseDetail(null)} aria-label="Close">×</button><div className="transaction-head"><span className="order-no">{expenseDetail.kind === "order" ? "Order-wise expenses" : "Person-wise expenses"}</span><h2>{expenseDetail.title}</h2><p>{detailExpenses.length} related expense record{detailExpenses.length === 1 ? "" : "s"} across {detailOrderCount} order{detailOrderCount === 1 ? "" : "s"}.</p></div><div className="transaction-summary"><div><span>Total amount</span><strong>{fmt(detailExpenses.reduce((sum, expense) => sum + expense.amount, 0))}</strong></div><div><span>Pending reimbursement</span><strong className={detailPending ? "negative" : "positive"}>{fmt(detailPending)}</strong></div><div><span>Paid</span><strong className="positive">{fmt(detailReimbursed)}</strong></div></div><div className="drawer-section"><h3>Related expense records and payment status</h3><div className="transaction-list">{detailExpenses.map((expense) => { const order = orderById(expense.orderId); const supervisorExpense = workflowExpenseById.get(expense.id); const pendingAmount = supervisorExpense?.status === "Approved" ? reimbursementPending(supervisorExpense.amount, supervisorExpense.reimbursedAmount) : 0; const paymentStatus = !supervisorExpense ? "Recorded expense" : supervisorExpense.status === "Rejected" ? "Rejected" : supervisorExpense.status === "Pending approval" ? "Awaiting approval" : supervisorExpense.status === "Paid" || !pendingAmount ? "Paid" : "Pending reimbursement"; return <div className="transaction-item" key={expense.id}><span className="activity-icon">₹</span><div className="grow"><strong>{expense.description || expense.category}</strong><span>{order?.orderNo || "Order"} · {shortDate(expense.expenseDate)} · {paymentStatus}</span><small>{expense.category}{supervisorExpense ? ` · ${fmt(supervisorExpense.reimbursedAmount)} paid` : ""}</small></div><strong>{fmt(expense.amount)}</strong></div>; })}{!detailExpenses.length && <div className="mini-empty">No related expenses match the current date filter.</div>}</div></div></aside></div>}
    {reviewExpense && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setReviewExpense(null)}><div className="record-modal reimbursement-modal" role="dialog" aria-modal="true" aria-label="Review reimbursement request"><div className="modal-head"><div><span className="overline">Reimbursement request</span><h2>{reviewExpense.supervisorName}</h2><p>{reviewExpense.orderNo} · {reviewExpense.orderTitle || "Order expense"}</p></div><button type="button" className="modal-close" onClick={() => setReviewExpense(null)} aria-label="Close reimbursement request">×</button></div><div className="reimbursement-review-body"><div className="reimbursement-review-grid"><div><span>Expense date</span><strong>{shortDate(reviewExpense.expenseDate)}</strong></div><div><span>Amount</span><strong>{fmt(reviewExpense.amount)}</strong></div><div><span>Category</span><strong>{reviewExpense.category}</strong></div><div><span>Payment mode</span><strong>{reviewExpense.paymentMode}</strong></div><div><span>Submitted by</span><strong>{reviewExpense.submittedByName || "Legacy supervisor"}</strong><small>{reviewExpense.submittedByRole || "supervisor"}</small></div><div><span>Status</span><Status value={reviewExpense.status} /></div></div>{reviewExpense.description && <div className="reimbursement-review-note"><span>Description</span><p>{reviewExpense.description}</p></div>}<div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={() => setReviewExpense(null)}>Close</button>{user.role === "admin" && reviewExpense.status === "Pending approval" && <><button type="button" className="btn btn-danger" disabled={Boolean(workflowWorking)} onClick={() => void updateSupervisorExpense(reviewExpense, "reject")}>Reject</button><button type="button" className="btn btn-primary" disabled={Boolean(workflowWorking)} onClick={() => void updateSupervisorExpense(reviewExpense, "approve")}>Accept</button></>}{(user.role === "admin" || user.role === "accountant") && reviewExpense.status === "Approved" && reimbursementPending(reviewExpense.amount, reviewExpense.reimbursedAmount) > 0 && <button type="button" className="btn btn-primary" disabled={Boolean(workflowWorking)} onClick={() => { setReimbursementExpense(reviewExpense); setReimbursementFile(null); setReviewExpense(null); }}>Mark paid</button>}</div></div></div></div>}
    {reimbursementExpense && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setReimbursementExpense(null)}><div className="record-modal reimbursement-modal" role="dialog" aria-modal="true" aria-label="Mark reimbursement paid"><div className="modal-head"><div><span className="overline">Reimbursement</span><h2>Mark as paid</h2><p>{reimbursementExpense.supervisorName} · {reimbursementExpense.orderNo} · {fmt(reimbursementPending(reimbursementExpense.amount, reimbursementExpense.reimbursedAmount))}</p></div><button type="button" className="modal-close" onClick={() => setReimbursementExpense(null)} aria-label="Close reimbursement form">×</button></div><form onSubmit={reimburseSupervisorExpense}><div className="form-grid"><Field label="Payment account *"><select name="paymentAccountId" required defaultValue={data.paymentAccounts[0]?.id || ""}><option value="">{data.paymentAccounts.length ? "Select account" : "Add an account in Settings first"}</option>{data.paymentAccounts.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select></Field><Field label="Method *"><select name="method" required defaultValue="Bank transfer"><option>Bank transfer</option><option>UPI</option><option>Cash</option><option>Cheque</option><option>Credit card</option></select></Field><Field label="Reference"><input name="reference" placeholder="UTR, cheque no. or reference" /></Field><FileField file={reimbursementFile} setFile={setReimbursementFile} label="Attach payment receipt (optional)" /></div><div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={() => setReimbursementExpense(null)}>Cancel</button><button type="submit" className="btn btn-primary" disabled={Boolean(workflowWorking) || !data.paymentAccounts.length}>{workflowWorking ? "Recording…" : "Mark paid"}</button></div></form></div></div>}
...
function RecordModal({ kind, data, user, preferredVendorId, editingCustomer, editingVendor, editingOrder, editingPayment, editingVendorProduct, file, setFile, error, saving, onClose, onSubmit }: { kind: Exclude<ModalKind, null>; data: AppData; user: PublicUser; preferredVendorId: string; editingCustomer: Customer | null; editingVendor: Vendor | null; editingOrder: Order | null; editingPayment: Payment | null; editingVendorProduct: VendorProduct | null; file: File | null; setFile: (file: File | null) => void; error: string; saving: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const [expenseFunding, setExpenseFunding] = useState<ExpenseFundingSource>("Reimbursement");
  ...
  const eligibleExpensePeople = data.persons.filter((person) => isExpenseResponsiblePerson(person, selectedExpenseOrder));
    ...
    {kind === "expense" && <><Field label="Order *"><OrderSelect orders={data.orders} name="orderId" value={expenseOrderId} onChange={setExpenseOrderId} showContext /></Field>{user.role !== "supervisor" && <Field label="Person responsible *"><PersonSelect persons={eligibleExpensePeople} name="personId" emptyLabel={expenseOrderId ? "No eligible active person for this order" : "Select an order first"} /></Field>}<Field label="Expense date *"><input name="expenseDate" required type="date" defaultValue={today()} /></Field><Field label="Category *"><select name="category" required>{availableExpenseCategories.map((category) => <option key={category}>{category}</option>)}</select></Field><Field label="Amount *"><input name="amount" required type="number" min="1" placeholder="0" /></Field><Field label="Payment mode"><select name="paymentMode"><option>UPI</option><option>Bank transfer</option><option>Cash</option><option>Credit card</option><option>Cheque</option></select></Field><div className="field field-wide funding-source-field"><span>How was this expense funded? *</span><div className="funding-source-options"><label className={expenseFunding === "Reimbursement" ? "selected" : ""}><input type="radio" name="fundingSource" value="Reimbursement" checked={expenseFunding === "Reimbursement"} onChange={() => setExpenseFunding("Reimbursement")} /><strong>Send for reimbursement</strong><small>Admin approval, pending and paid status will be tracked.</small></label><label className={expenseFunding === "Account" ? "selected" : ""}><input type="radio" name="fundingSource" value="Account" checked={expenseFunding === "Account"} onChange={() => setExpenseFunding("Account")} /><strong>Paid from company account</strong><small>Record against Hope and Dream, eRentals or another configured account.</small></label></div></div>{expenseFunding === "Account" && <Field label="Company account *"><select name="paymentAccountId" required defaultValue={data.paymentAccounts[0]?.id || ""}><option value="">{data.paymentAccounts.length ? "Select account" : "Add an account in Settings first"}</option>{data.paymentAccounts.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select></Field>}<Field label="Description" wide><textarea name="description" rows={2} placeholder="What was this expense for?" /></Field><FileField file={file} setFile={setFile} label="Attach receipt (optional)" /></>}
    {kind === "payment" && <>
      ...
      <Field label="Account *"><select name="paymentAccountId" required defaultValue={editingPayment?.paymentAccountId || data.paymentAccounts[0]?.id || ""}><option value="">{data.paymentAccounts.length ? "Select account" : "Add an account in Settings first"}</option>{data.paymentAccounts.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select></Field>
      ...
      <FileField file={file} setFile={setFile} label="Attach payment receipt (optional)" />
      {editingPayment?.receiptKey && <a className="file-link field-wide" href={`/api/upload?key=${encodeURIComponent(editingPayment.receiptKey)}`} target="_blank" rel="noreferrer">▤ View current {editingPayment.receiptName || "payment receipt"}</a>}
    </>}
